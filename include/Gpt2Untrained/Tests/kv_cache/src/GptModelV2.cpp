...
GptModelV2Impl::GptModelV2Impl(config& cfg) : cfg(cfg)
{
   ...
   auto lnShape = {static_cast<int64_t>(cfg.emb_dim)};
    ...
    trfBlock = register_module("trfBlock", [&cfg]()
        {
...
torch::Tensor GptModelV2Impl::forward(torch::Tensor x, bool use_cache) {
    ...
    if (use_cache)
    {
        ...
        if (auto context_length = postembed->options.num_embeddings();
            current_ptr + seq_length > context_length) {
            throw std::invalid_argument("Position embedding overflow.");
        }
